    ...
    this.opportunityservice.getUserOpportunities(this.id).subscribe((val) => {
      val.forEach((applicant) => {
        ...
        this.opportunityservice
          .getvolunteer(this.userdetails?.id + '', applicant.id)
          .subscribe((v) => {
            this.firestore
              .collection<apply>('oportunities/' + applicant.id + '/applicants')
              .doc(v[0].id)
              .update({ userdetails: this.userdetails });
          });
      });
    });
  ...
  addCourse(obj: any) {
    this.courses.push(
      new FormGroup({
        ...
        hours: new FormControl(obj.hours, [
          Validators.required,
          Validators.min(0),
        ]),
      })
